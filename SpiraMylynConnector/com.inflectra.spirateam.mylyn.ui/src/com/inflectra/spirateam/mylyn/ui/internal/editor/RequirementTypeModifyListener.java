		...
		try
		{
			...
			if (editorPage != null)
			{
				...
				if (connector != null)
				{
					...
					if (dataHandler != null)
					{
						Set<TaskAttribute> changedAttributes = dataHandler.changeRequirementType(repository, attributeEditor.getTaskAttribute().getTaskData(), newIncidentTypeName);
						...
						for (TaskAttribute changedAttribute : changedAttributes)
						{
							...
							if (!changedAttribute.getId().equals(ArtifactAttribute.REQUIREMENT_TYPE.getArtifactKey()))
							{
