		...
		try
		{
			...
			if (editorPage != null)
			{
				...
				if (connector != null)
				{
					...
					if (dataHandler != null)
					{
						Set<TaskAttribute> changedAttributes = dataHandler.changeTaskType(repository, attributeEditor.getTaskAttribute().getTaskData(), newIncidentTypeName);
						...
						for (TaskAttribute changedAttribute : changedAttributes)
						{
							...
							if (!changedAttribute.getId().equals(ArtifactAttribute.TASK_TYPE.getArtifactKey()))
							{
